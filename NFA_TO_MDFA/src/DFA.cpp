...
void push_start_state(queue <set<int>>& cur_dfa_states, map <set <int>, int>& dfa_numeration,
    int& num_of_node_in_dfa, set <set <int>>& processed, vector <vector <int>>& dfa_transitions, int alph_sz) {
    cur_dfa_states.push({ 1 });
    dfa_numeration[{1}] = num_of_node_in_dfa;
    num_of_node_in_dfa++;
    processed.insert({ 1 });
    vector <int> letters(alph_sz);
    dfa_transitions.push_back(letters);
}
...
void check_if_new(set <set <int>>& processed, set <int>& new_state, set <int>& cur_state,
    ...
    vector <vector <int>>& dfa_transitions, int letter) {
    ...
    dfa_transitions[dfa_numeration[cur_state]][letter] = dfa_numeration[new_state];
}
    ...
    for (auto compound_state : processed) {
        for (auto final_state : final_states) {
            if (compound_state.find(final_state) != compound_state.end()) {
                dfa_final_states.insert(dfa_numeration[compound_state]);
            }
        }
    }
}
